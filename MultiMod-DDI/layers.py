...
class RGCNLayer(nn.Module):
    def __init__(self, inp_dim, out_dim, aggregator, bias=None, activation=None, num_nodes=122343142,dropout=0.0, 
        edge_dropout=0.0, is_input_layer=False, embed=False, add_transe_emb=True, args=None):
...
class RGCNBasisLayer(RGCNLayer):
    def __init__(self, inp_dim, out_dim, aggregator, attn_rel_emb_dim, num_rels,num_bases=-1,  num_nodes = 12345342, bias=None, has_kg = True,
                ...
                one_attn = False, A1 = None, A2 = None, gamma= 0.0, args = None):
        super(
            RGCNBasisLayer,
            self).__init__(
            inp_dim,
        ...
        self.FiLM_layer = nn.Linear(in_features=self.out_dim,
                                    out_features=2 * self.out_dim,
                                    bias=False)
    ...
    def propagate(self, g, attn_rel_emb = None, nonKG = True):
        ...
        weight = torch.matmul(self.w_comp, weight).view(
            self.num_rels, self.inp_dim, self.out_dim).to(self.args.device)
        ...
        def msg_func(edges):
            ...
            film_weights = self.FiLM_layer(self.embed)
            message_film_weights = film_weights[edges.dst['idx']]

            message_film_gamma = message_film_weights[:,:self.out_dim]
            message_film_beta = message_film_weights[:,self.out_dim:]
            ...
            if self.has_attn:
                ...
                import math
                x = torch.cat( [edges.src['feat'], self.embed[edges.src['idx']]], dim = 1)  # 源节点特征+嵌入
                y = torch.cat( [edges.dst['feat'], self.embed[edges.dst['idx']]], dim = 1)  # 目标节点特征+嵌入
                x_hat = self.A1(x)  # hj * Wj
                y_hat = self.A2(y)+attn_rel_emb(edges.data['type'])  # hi * Wi + rij
            ...
            return {'curr_emb': curr_emb, 'msg': msg, 'gamma': message_film_gamma, 'beta': message_film_beta, 'alpha': a}
