...
def extract_fingerprints(radius, atoms, i_jbond_dict,
    ...
    if (len(atoms) == 1) or (radius == 0):
        ...
        for _ in range(radius):
            ...
            The updated node IDs are the fingerprint_file IDs.
            """
            nodes_ = []
...
def create_datasets(corpus_tsv_dir, database_tsv_dir, radius):
    ...
    if corpus_tsv_dir != 'none':
        ...
        corpus_test = create_dataset(os.path.join(corpus_tsv_dir, 'test.tsv'))  # 新增测试集处理
        outputs_dict['corpus_train'] = corpus_train
        outputs_dict['corpus_dev'] = corpus_dev
        outputs_dict['corpus_test'] = corpus_test  # 添加测试集到输出字典
    ...
    if database_tsv_dir != 'none':
        ...
        database_test = create_dataset(os.path.join(database_tsv_dir, 'test.tsv'))
        outputs_dict['database_train'] = database_train
        outputs_dict['database_dev'] = database_dev
        outputs_dict['database_test'] = database_test
...
corpus_tsv_dir=r"E:\pythonProjects\自己重新搭建\包含知识子图的修改后代码\IMSE-mainME2\dataset\DDI_corpus"
database_tsv_dir='none'
radius=1
output_dir= 'fingerprint_file'
...
for k,v in outputs_dict.items():
    np.save(os.path.join(output_dir, k), np.asarray(v,dtype=object))  #即.npy
