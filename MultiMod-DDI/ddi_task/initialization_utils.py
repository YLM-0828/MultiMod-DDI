...
def initialize_experiment(params, file_name):
    ...
    '''
    # params.work_dir = os.path.join(os.path.relpath(os.path.dirname(os.path.abspath(__file__))), '..')
    exps_dir = os.path.join(params.work_dir, 'experiments')
...
def initialize_model(params, model, load_model=False):
    ...
    if load_model and os.path.exists(os.path.join(params.exp_dir, 'best_graph_classifier.pth')):
        ...
        relation2id_path = os.path.join(params.work_dir, f'data/{params.dataset}/relation2id.json')
