...
import pickle
import dgl
from graph_utils import ssp_multigraph_to_dgl, incidence_matrix, deserialize
from data_utils import process_files, process_files_ddi, save_to_file, plot_rel_dist,process_files_decagon
...
def generate_subgraph_datasets(args, splits=['total'], saved_relation2id=None, max_label_value=None):
# def generate_subgraph_datasets(params, splits=['total'], saved_relation2id=None, max_label_value=None):
    ...
    triple_file = os.path.join(args.work_dir,'data/{}/relations_{}hop.txt'.format(args.dataset,args.hop)) 
    if args.dataset == 'drugbank':
        adj_list, triplets, entity2id, relation2id, id2entity, id2relation, rel = process_files_ddi(args.file_paths, triple_file, saved_relation2id)
    else:
        adj_list, triplets, entity2id, relation2id, id2entity, id2relation, rel, triplets_mr, polarity_mr = process_files_decagon(args.file_paths, triple_file, saved_relation2id)
    ...
    data_path = os.path.join(args.work_dir, f'data/{args.dataset}/relation2id.json')
    ...
    for split_name in splits: # ['train', 'valid', 'test']
        if args.dataset == 'drugbank':
            #  graphs = {"train":{"triplets":[[],[],[],...],"max_size":250000},"valid":{}},......}
            graphs[split_name] = {'triplets': triplets[split_name], 'max_size': args.max_links}
        elif args.dataset == 'BioSNAP':
            graphs[split_name] = {'triplets': triplets_mr[split_name], 'max_size': args.max_links, "polarity_mr": polarity_mr[split_name]}
    # Sample train and valid/test links
    for split_name, split in graphs.items():
        logging.info(f"Sampling negative links for {split_name}")
        split['pos'], split['neg'] = sample_neg(adj_list, split['triplets'], args.num_neg_samples_per_link, max_size=split['max_size'], constrained_neg_prob=args.constrained_neg_prob)
        # graphs = {"train" : {"triplets":[[],[],[],...] , "max_size":250000 , "pos":[[d1,d2,r],[],[],...], "neg":[[d1,d2,r],[],[],...] }，
        #           "valid" : 
        #           "test"  :                                                                                                           }
    #print(graphs.keys())
    if testing:
        directory = os.path.join(args.work_dir, 'data/{}/'.format(args.dataset))
        save_to_file(directory, f'neg_{args.test_file}_{args.constrained_neg_prob}.txt', graphs['test']['neg'], id2entity, id2relation)
...
class SubgraphDataset(Dataset):
    ...
    def __init__(self, db_path, db_name_pos, db_name_neg, raw_data_paths, included_relations=None, add_traspose_rels=False, num_neg_samples_per_link=1, use_kge_embeddings=False, dataset='', kge_model='',  \
        ssp_graph = None,  relation2id= None, id2entity= None, id2relation= None, rel= None,  graph = None, morgan_feat = None, file_name='', args=""):
        ...
        self.entity_type = np.loadtxt(os.path.join(args.work_dir,'data/{}/entity.txt'.format(dataset)))
        ...
        if not ssp_graph:
            ...
            data_path =  os.path.join(args.work_dir,'data/{}/relation2id.json'.format(dataset))
            #print(os.pwd)
    ...
    def __getitem__(self, index):
        ...
        # return tuple(tensor[index] for tensor in self.tensors), subgraph_pos, g_label_pos, r_label_pos
    ...
    def _prepare_subgraphs(self, nodes, r_label, n_labels):
        ...
        # subgraph = dgl.DGLGraph(self.graph.node_subgraph(nodes))
